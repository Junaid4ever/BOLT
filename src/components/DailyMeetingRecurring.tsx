...
export function DailyMeetingRecurring({ isDark, onSubmit, onCancel }: DailyMeetingRecurringProps) {
  ...
  const toggleExceptionDay = (day: string) => {
      ...
      <div className={`rounded-2xl shadow-2xl max-w-md w-full p-6 ${
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map(day => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={exceptionDays.includes(day)}
                    onChange={() => toggleExceptionDay(day)}
            ...
            <p className={`text-xs mt-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              Meeting will NOT be scheduled on selected days
            </p>
          </div>
