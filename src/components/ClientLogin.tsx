...
import { LogIn, UserCircle, Sparkles, Lock, Mail, ArrowRight, Upload, CheckCircle, AlertCircle } from 'lucide-react';
...
export function ClientLogin({ onLogin }: ClientLoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (byEmail) {
        ...
        const usernameClean = email.toLowerCase().replace(/\s+/g, '');
        const prefixMatch = email.match(/^([A-Za-z]+)[_-](.+)$/);
        const displayName = prefixMatch ? prefixMatch[2] : email;
        const displayNameClean = displayName.toLowerCase().replace(/\s+/g, '');

        const emailVariants = [
          `${usernameClean}@client.prozoom.com`,
          `${usernameClean}@client.junaid.com`,
          `${displayNameClean}@client.junaid.com`,
          `${displayNameClean}@client.prozoom.com`
        ];

        for (const tryEmail of emailVariants) {
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    if (!paymentScreenshot || !paymentAmount) {
      alert('Please provide both payment amount and screenshot');
      return;
    }
    ...
    try {
      ...
      const fileName = `${blockedUserId}-${Date.now()}.${fileExt}`;
      ...
      await supabase.from('payments').insert({
        client_id: blockedUserId,
        client_name: blockedUserName,
        ...
        <div className="relative min-h-screen flex items-center justify-center p-4">
          <div className="w-full max-w-md">
            <div className="relative group animate-fade-in-up-delay">
              <div className="absolute -inset-0.5 bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 rounded-3xl blur-lg opacity-60 group-hover:opacity-80 transition duration-1000"></div>
              <div className="relative bg-slate-900/90 backdrop-blur-xl rounded-3xl p-8 border border-slate-800 shadow-2xl">
                <div className="text-center">
                  ...
                  <h2 className="text-2xl font-black text-white mb-4">Payment Submitted!</h2>
                  <p className="text-slate-300 mb-6 leading-relaxed">
                    Payment submitted, waiting for admin approval. Post approval your panel will be unblocked.
                  </p>
                  <button
                    onClick={handleBackToLogin}
                    className="w-full px-6 py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold rounded-xl hover:scale-105 transition-all duration-300"
                  >
                    Back to Login
                  </button>
                </div>
    ...
    return (
      <div className="min-h-screen relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-red-950 via-orange-950 to-amber-950">
          <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-20"></div>
        </div>

        <div className="relative min-h-screen flex items-center justify-center p-4">
          <div className="w-full max-w-md">
            <div className="relative group animate-fade-in-up-delay">
              <div className="absolute -inset-0.5 bg-gradient-to-r from-red-500 via-orange-500 to-amber-500 rounded-3xl blur-lg opacity-60 group-hover:opacity-80 transition duration-1000"></div>
              <div className="relative bg-slate-900/90 backdrop-blur-xl rounded-3xl p-8 border border-slate-800 shadow-2xl">
                ...
                <h2 className="text-2xl font-black text-white mb-2 text-center">Login Disabled</h2>
                <p className="text-red-400 font-semibold mb-6 text-center">Please pay your dues to unlock panel</p>
                ...
                <form onSubmit={handlePaymentSubmit} className="space-y-5">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3.5 rounded-xl bg-slate-800/50 border-2 border-slate-700 text-white placeholder-slate-500 focus:border-orange-500 focus:bg-slate-800 outline-none transition-all duration-300"
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <label
                        ...
                        className="flex items-center justify-center gap-3 w-full px-4 py-3.5 rounded-xl bg-slate-800/50 border-2 border-slate-700 text-slate-400 hover:border-orange-500 hover:text-white cursor-pointer transition-all duration-300"
                      >
                  ...
                  <button
                    ...
                    className="w-full px-6 py-4 bg-gradient-to-r from-orange-600 to-amber-600 text-white font-bold rounded-xl hover:scale-105 transition-all duration-300 disabled:opacity-50"
                  >
                    {isSubmittingPayment ? 'Submitting...' : 'Submit Payment for Approval'}
                  </button>

                  <button
                    type="button"
                    onClick={handleBackToLogin}
                    className="w-full px-6 py-3 bg-slate-800 text-slate-300 font-semibold rounded-xl hover:bg-slate-700 transition-all duration-300"
                  >
                    Back to Login
                  </button>
                </form>
      ...
      <div className="relative min-h-screen flex items-center justify-center p-4">
        <div className="w-full max-w-md">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <input
                      ...
                      placeholder="VD_Panchal or email@example.com"
                      required
                    />
          ...
          <div className="mt-6 text-center text-xs text-slate-500 animate-fade-in">
            <p>Prozoom Services Management System v2.0</p>
          </div>
        </div>
      </div>
