...
import { Plus, LogOut, Clock, Users, Trash2, Image as ImageIcon, Loader, Moon, Sun, Calendar, AlertTriangle, Eye, EyeOff, Key, ArrowLeft, ArrowRight, FileText, RefreshCw, X } from 'lucide-react';
...
export function ClientPanel({ user, onLogout }: ClientPanelProps) {
  ...
  const handleOpenAll = () => {
    ...
    const meetingsToOpen = filteredMeetings
      .filter(m => !m.attended && m.status !== 'cancelled')
      .sort((a, b) => {
  ...
  const fetchDailyDues = async () => {
    ...
    if (!error && data) {
      ...
      console.log('Client: Dues till today:', duesTillToday, 'Total:', totalDueTillToday, 'Is past 9:30 PM IST:', isPast930PM);
    ...
    <div className={`min-h-screen transition-colors duration-300 ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          ...
          {showRecurringModal && (
            <DailyMeetingRecurring
              ...
              onSubmit={async (recurringData) => {
                ...
                if (recurringData.exceptionDays && recurringData.exceptionDays.length > 0) {
                  const exceptions = recurringData.exceptionDays.map(day => ({
                    recurring_meeting_id: newRecurringMeeting.id,
                    exception_day: day
                  }));
                ...
                const todayMeeting = {
                  ...
                  is_instant: isInstant,
                  recurring_id: newRecurringMeeting.id
                };
          ...
          <div className="mt-4">
            <button
              ...
              className={`w-full px-4 py-3 rounded-xl font-bold transition-all flex items-center justify-center gap-2 shadow-lg hover:shadow-xl ${
                showScheduleView
          ...
          {meetings.length > 0 && (
            ...
            {(() => {
              ...
              if (attendedMeetingsWithScreenshots.length > 0 || dateAdjustments.length > 0) {
                const totalMembersForDate = attendedMeetingsWithScreenshots.reduce((sum, m) => sum + (m.member_count || 0), 0);
                  ...
                  <div ref={breakdownSectionRef} className="mb-4">
                    <div className={`border-2 rounded-xl p-4 shadow-md ${
                      isDark
                        ...
                        <button
                          onClick={() => setShowMeetingBreakdown(!showMeetingBreakdown)}
                          className={`w-full text-left text-lg font-bold mb-3 flex items-center justify-between gap-2 hover:opacity-80 transition-opacity ${
                            isDark ? 'text-white' : 'text-gray-900'
                          }`}
                        >
                          <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm">{showMeetingBreakdown ? '▲ Hide' : '▼ Show'}</span>
                        </button>

                        {showMeetingBreakdown && attendedMeetingsWithScreenshots.length > 0 && (() => {
                          const totalMembers = totalMembersForDate;
                        ...
                        {showMeetingBreakdown && dateAdjustments.length > 0 && (
                        ...
                        <div className={`border-t-2 pt-4 mt-4 ${isDark ? 'border-slate-600' : 'border-teal-300'}`}>
                          <div className={`flex justify-between items-center p-3 rounded-xl ${isDark ? 'bg-slate-700/50' : 'bg-teal-50'}`}>
                            <span className={`text-lg font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Total Members</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 max-h-[400px] md:max-h-[500px] overflow-y-auto">
              {[...getFilteredMeetings()].sort((a, b) => {
                ...
                const cardGradients = meeting.is_instant
                  ? {
                      blur: 'bg-gradient-to-br from-amber-400 to-red-400',
                      shadow: 'bg-gradient-to-br from-amber-300 to-orange-300',
                      main: 'bg-gradient-to-br from-amber-500 via-orange-500 to-red-500'
                    }
                  : isDark
                  ? [
                      { blur: 'bg-gradient-to-br from-blue-600/80 to-cyan-800/80', shadow: 'bg-gradient-to-br from-blue-600 to-cyan-700', main: 'bg-gradient-to-br from-blue-700 via-blue-600 to-cyan-700' },
                      { blur: 'bg-gradient-to-br from-emerald-600/80 to-teal-800/80', shadow: 'bg-gradient-to-br from-emerald-600 to-teal-700', main: 'bg-gradient-to-br from-emerald-700 via-teal-600 to-green-700' },
                      { blur: 'bg-gradient-to-br from-orange-600/80 to-amber-800/80', shadow: 'bg-gradient-to-br from-orange-600 to-amber-700', main: 'bg-gradient-to-br from-orange-700 via-amber-600 to-yellow-700' },
                      { blur: 'bg-gradient-to-br from-rose-600/80 to-pink-800/80', shadow: 'bg-gradient-to-br from-rose-600 to-pink-700', main: 'bg-gradient-to-br from-rose-700 via-pink-600 to-red-700' },
                      { blur: 'bg-gradient-to-br from-violet-600/80 to-purple-800/80', shadow: 'bg-gradient-to-br from-violet-600 to-purple-700', main: 'bg-gradient-to-br from-violet-700 via-purple-600 to-fuchsia-700' }
                    ][index % 5]
                  : [
                      { blur: 'bg-gradient-to-br from-blue-300/80 to-cyan-400/80', shadow: 'bg-gradient-to-br from-blue-400 to-cyan-500', main: 'bg-gradient-to-br from-blue-500 via-blue-600 to-cyan-600' },
                      { blur: 'bg-gradient-to-br from-emerald-300/80 to-teal-400/80', shadow: 'bg-gradient-to-br from-emerald-400 to-teal-500', main: 'bg-gradient-to-br from-emerald-500 via-teal-600 to-green-600' },
                      { blur: 'bg-gradient-to-br from-orange-300/80 to-amber-400/80', shadow: 'bg-gradient-to-br from-orange-400 to-amber-500', main: 'bg-gradient-to-br from-orange-500 via-amber-600 to-yellow-600' },
                      { blur: 'bg-gradient-to-br from-rose-300/80 to-pink-400/80', shadow: 'bg-gradient-to-br from-rose-400 to-pink-500', main: 'bg-gradient-to-br from-rose-500 via-pink-600 to-red-600' },
                      { blur: 'bg-gradient-to-br from-violet-300/80 to-purple-400/80', shadow: 'bg-gradient-to-br from-violet-400 to-purple-500', main: 'bg-gradient-to-br from-violet-500 via-purple-600 to-fuchsia-600' }
                    ][index % 5];
                ...
                >
                  <div className={`absolute inset-0 rounded-xl blur-xl opacity-30 ${cardGradients.blur}`} style={{ transform: 'translateY(8px) scale(0.95)' }} />

                  <div className={`absolute inset-0 rounded-xl ${cardGradients.shadow}`} style={{ transform: 'translateY(4px) scale(0.98)' }} />
                  ...
                  >
                    <div className={`absolute inset-0 ${cardGradients.main}`} />
                    ...
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-white/30 backdrop-blur-sm rounded-lg text-sm font-bold">
                            <Users size={14} />
                            {user.name}
                          </span>
                        </div>
      ...
      {showScheduleView && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={`w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl border ${
            isDark
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="px-3 py-1 rounded-full text-xs font-bold bg-purple-500 text-white">
                              {new Date(meeting.scheduled_date || meeting.created_at).toLocaleDateString()}
                            </span>
                          </div>
