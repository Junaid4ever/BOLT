...
export function NotificationSystem({ userId, onNotificationClick }: NotificationSystemProps) {
  ...
  const getNotificationBg = (type: string, isRead: boolean) => {
    ...
    switch (type) {
      case 'success':
        return 'bg-gradient-to-r from-green-50 to-emerald-50 border-green-400 shadow-green-100 shadow-md';
      case 'error':
        return 'bg-gradient-to-r from-red-50 to-rose-50 border-red-400 shadow-red-100 shadow-md';
      case 'warning':
        return 'bg-gradient-to-r from-orange-50 via-amber-50 to-yellow-50 border-orange-400 shadow-orange-100 shadow-lg animate-pulse';
      default:
        return 'bg-gradient-to-r from-blue-50 to-cyan-50 border-blue-400 shadow-blue-100 shadow-md';
    }
  };
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p className={`text-sm whitespace-pre-line ${notification.is_read ? 'text-gray-600' : 'text-gray-900 font-semibold'}`}>
                          ...
                          {!notification.is_read && (
                            <span className="inline-block mt-2 px-3 py-1 bg-gradient-to-r from-purple-600 to-pink-600 text-white text-xs font-bold rounded-full animate-pulse">
                              NEW
                            </span>
                          )}
                          {notification.type === 'warning' && !notification.is_read && (
                            <p className="text-xs text-orange-600 font-bold mt-2 animate-pulse">
                              ⚠️ Click to acknowledge this important message
                            </p>
                          )}
                        </div>
                      </div>
