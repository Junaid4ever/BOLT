...
interface HistoricalMeeting {
  ...
  time_period?: 'AM' | 'PM';
  member_count?: number;
  member_type?: 'indians' | 'foreigners';
  client_name: string;
...
export function CalendarView({ onClose, isDark = false, onDateSelect }: CalendarViewProps) {
  ...
  const fetchHistoricalMeetings = async () => {
    ...
    if (!historicalResult.error && historicalResult.data) {
      const historicalWithType = historicalResult.data.map(m => ({
        ...m,
        member_type: m.member_type || 'indians'
      }));
      allMeetingsData.push(...historicalWithType);
    }
  ...
  const organizeByClient = async () => {
    const { data: users } = await supabase
      .from('users')
      .select('id, name, price_per_member, foreign_member_rate')
      .eq('role', 'client');
    ...
    const { data: dailyDues } = await supabase
      .from('daily_dues')
      .select('client_id, date, due_amount')
      .gte('date', startDate)
      .lte('date', endDate);
    ...
    for (const user of users) {
      ...
      const clientDailyDues = dailyDues?.filter(d => d.client_id === user.id) || [];
      const totalFromDues = clientDailyDues.reduce((sum, d) => sum + (d.due_amount || 0), 0);

      const totalAmount = totalFromDues > 0 ? totalFromDues : clientMeetings.reduce((sum, m) => {
        const memberCount = m.member_count || 0;
        const memberType = m.member_type || 'indians';
        const rate = memberType === 'foreigners'
          ? (user.foreign_member_rate || user.price_per_member || 0)
