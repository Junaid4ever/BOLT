...
import { Users, IndianRupee, Calendar, TrendingUp, Bell, UserCog, Key, Trash2, LogIn, Eye, DollarSign, CheckCircle, XCircle, TrendingDown, Star, Copy, Search } from 'lucide-react';
import { useTheme } from '../contexts/ThemeContext';
import { isClientImportant, toggleImportantClient } from '../utils/importantClients';
import { getUsdtRate, setUsdtRate } from '../utils/usdtRateStorage';
...
export function ClientsOverview() {
  ...
  const fetchClientsData = async () => {
    ...
    const allClientNames = [...clientNames, ...subClientNames];

    const subClientToParent = new Map<string, { parentName: string; parentId: string }>();
    subClients?.forEach(s => {
      const parent = usersData.find(u => u.id === s.parent_user_id);
      if (parent) {
        subClientToParent.set(s.name, { parentName: parent.name, parentId: parent.id });
      }
    });

    const [dailyDuesResult, paymentsResult, advancesResult, rejectedPaymentsResult, subclientMeetingsResult] = await Promise.all([
      supabase
        .from('daily_dues')
        .select('client_id, client_name, date, amount')
      ...
      supabase
        .from('payments')
        .select('client_id, rejected_amount')
        .in('client_id', clientIds)
        .eq('status', 'rejected'),
    ...
    rejectedPaymentsResult.data?.forEach(payment => {
      const existing = paymentsByClient.get(payment.client_id) || { total: 0, rejected: 0, lastPayment: null };
    ...
    const duesByClientId = new Map<string, { date: string; amount: number }[]>();
    dailyDuesResult.data?.forEach(due => {
      const existing = duesByClientId.get(due.client_id) || [];
      existing.push({ date: due.date, amount: Number(due.amount) || 0 });
      duesByClientId.set(due.client_id, existing);
    });

    const subclientDuesByParent = new Map<string, { date: string; amount: number }[]>();
    const todayStr = new Date().toISOString().split('T')[0];
    subclientMeetingsResult.data?.forEach(meeting => {
      const parentInfo = subClientToParent.get(meeting.client_name);
      ...
      const parent = usersData.find(u => u.id === parentInfo.parentId);
      if (!parent) return;

      const parentPricePerMember = Number(parent.price_per_member) || 1;
      const parentDpRate = Number(parent.dp_member_price) || Number(parent.price_per_dp_member) || parentPricePerMember;
      const rate = meeting.member_type === 'dp' ? parentDpRate : parentPricePerMember;
      const amount = (meeting.member_count || 0) * rate;

      const existing = subclientDuesByParent.get(parentInfo.parentId) || [];
      existing.push({ date, amount });
      subclientDuesByParent.set(parentInfo.parentId, existing);
    });
    ...
    const clientSummaries: ClientSummary[] = usersData.map(user => {
      ...
      const ownDues = duesByClientId.get(user.id) || [];
      const subclientDues = subclientDuesByParent.get(user.id) || [];
      const allDues = [...ownDues, ...subclientDues];

      const unpaidDues = allDues.filter(d =>
        !lastPaymentDate || d.date > lastPaymentDate
      );

      const unpaidDuesTotal = unpaidDues.reduce((sum, d) => sum + d.amount, 0);
      const uniqueDates = new Set(unpaidDues.map(d => d.date));
  ...
  const copyCredentials = (client: ClientSummary) => {
    const credentialsMessage = `Dear ${client.name},

Welcome to the world of PRO Zoom Automation Service!

We are delighted to have you on board. Below are your login credentials to access our professional Zoom meeting management platform:

🌐 Website: https://piano4ever.bolt.host
👤 Username: ${client.name}
🔑 Password: ${client.password_hash}

Please keep these credentials secure. You can now manage all your Zoom meetings effortlessly through our platform.

If you have any questions or need assistance, feel free to reach out.

Best regards,
Zoom Automation Team`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`rounded-2xl p-4 shadow-xl mb-6 ${isDark ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-gray-200'}`}>
        <div className="relative">
          ...
          <input
            ...
            className={`w-full pl-12 pr-4 py-3 rounded-xl border-2 focus:ring-2 focus:ring-emerald-500 outline-none font-semibold ${
              isDark
            ...
            <div className={`p-6 ${
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => {
                      toggleImportantClient(client.id);
                      fetchClientsData();
                    }}
                  ...
                  >
                    <Star
                      size={24}
                      className={isClientImportant(client.id) ? 'text-yellow-400 fill-yellow-400' : 'text-white/40'}
                    />
                  </button>
                  <div>
            ...
            <div className="p-6 space-y-4">
                ...
                <div className={`rounded-xl p-4 border-2 ${
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <DollarSign className={isDark ? 'text-blue-400' : 'text-blue-600'} size={20} />
                    ...
                    <p className={`text-2xl font-black ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                      ₹{formatIndianNumber(client.advanceAmount)}
                    </p>
                  </div>
                  <p className={`text-xs mt-2 ${isDark ? 'text-blue-300' : 'text-blue-600'}`}>
                    Auto-deducting from daily dues
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="col-span-2 flex items-center justify-center gap-2 bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white px-3 py-2.5 rounded-xl transition-all font-bold text-sm shadow-lg hover:scale-105"
                >
                  <Copy size={14} />
                  Copy Credentials
                </button>
                  ...
                  >
                    <span
                      className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform duration-300 ${
                        client.isBlocked ? 'translate-x-1' : 'translate-x-6'
                      }`}
                    />
                  </button>
                </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className={`rounded-2xl shadow-2xl max-w-md w-full p-8 ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={async () => {
                  const newRate = prompt(`Enter USDT Conversion Rate (INR per USDT):`, selectedClient.usdtRate.toString());
                  if (newRate && !isNaN(Number(newRate)) && Number(newRate) > 0) {
                    setUsdtRate(selectedClient.id, Number(newRate));
                    alert('USDT rate updated!');
                    fetchClientsData();
                    setShowRateModal(false);
                  } else if (newRate) {
                ...
                className="w-full flex items-center justify-between px-6 py-4 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white rounded-xl font-bold shadow-lg hover:scale-105 transition-all"
              >
                <span>USDT Conversion Rate</span>
                <span className="text-sm flex items-center gap-1">
                  <DollarSign size={16} />
                  {selectedClient.usdtRate} INR/USDT
                </span>
              </button>
            </div>
