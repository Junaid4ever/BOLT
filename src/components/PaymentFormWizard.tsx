...
import { IndianRupee, Wallet, QrCode, Upload, Check, ChevronRight, Calendar, Copy, Download, FileText, X, Clock, DollarSign } from 'lucide-react';
import { supabase } from '../lib/supabase';
import { getUsdtRate } from '../utils/usdtRateStorage';
...
export function PaymentFormWizard({ userId, userName, dailyDues, totalNetDue, onSuccess }: PaymentFormWizardProps) {
  ...
  useEffect(() => {
    ...
    setUsdtRate(getUsdtRate(userId));
  }, [userId]);
  ...
  const filterUnpaidDues = () => {
    ...
    const maxDate = isPast930PM ? today : new Date(new Date(today).getTime() - 24 * 60 * 60 * 1000).toISOString().split('T')[0];

    if (!lastPaidDate) {
      const filtered = dailyDues.filter(due => due.date <= maxDate);
      setUnpaidDues(filtered);
      return;
    }

    const filtered = dailyDues.filter(due => due.date > lastPaidDate && due.date <= maxDate);
    setUnpaidDues(filtered);
  };
  ...
  const fetchPaymentMethods = async () => {
    ...
    if (userData?.parent_user_id) {
      const { data } = await supabase
        ...
        .select('*')
        .eq('cohost_user_id', userData.parent_user_id)
        .maybeSingle();
    ...
    if (paymentData) {
      ...
      setUpiQrCode(paymentData.qr_code_url || '');
      setTrc20Address(paymentData.usdt_trc20_address || '');
      setTrc20QrCode(paymentData.usdt_trc20_qr || '');
      setBep20Address(paymentData.usdt_bep20_address || '');
      setBep20QrCode(paymentData.usdt_bep20_qr || '');
    }
  };
  ...
  const handleDateSelect = (date: string) => {
    ...
    if (isFullSettlement) {
      ...
      const duesUptoDate = unpaidDues.filter(d => d.date <= date);
      const cumulativeAmount = duesUptoDate.reduce((sum, d) => sum + Number(d.amount), 0);
        ...
        <div className="space-y-4 animate-fadeIn">
          ...
          <div className="grid gap-3 max-h-96 overflow-y-auto">
            {unpaidDues.length === 0 ? (
              ...
              (() => {
                const uniqueDates = [...new Set(unpaidDues.map(d => d.date))].sort();
                return uniqueDates.map((date) => {
                  const duesUptoDate = unpaidDues.filter(d => d.date <= date);
                  const cumulativeAmount = duesUptoDate.reduce((sum, d) => sum + Number(d.amount), 0);
                  const totalMeetings = duesUptoDate.reduce((sum, d) => sum + (d.meeting_count || 0), 0);
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-semibold text-gray-700">
                            Payment Upto {new Date(date).toLocaleDateString('en-IN', {
                              day: 'numeric',
                              month: 'short'
                            })}
                          </p>
        ...
        <div className="space-y-6 animate-fadeIn">
                ...
                {paymentMethod === 'crypto' && (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-lg font-bold text-amber-600">
                      {(paymentAmount / usdtRate).toFixed(2)} USDT
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-gray-200">
                <div className="bg-amber-50 rounded-lg p-2.5 border border-amber-200">
                  <p className="text-xs text-amber-800 font-medium text-center">
                    Conversion Rate: 1 USDT = ₹{usdtRate}
                  </p>
                </div>
              </div>
            )}
          ...
          {paymentMethod === 'crypto' && cryptoNetwork === 'trc20' && trc20Address && (
            <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-6 border-2 border-green-300">
              ...
              <div className="bg-gradient-to-r from-amber-500 to-orange-500 rounded-lg p-4 mb-4 text-center">
                <p className="text-sm text-white/90 mb-1">Send Exactly</p>
                ...
                <p className="text-xs text-white/80 mt-2">1 USDT = ₹{usdtRate}</p>
              </div>
