...
export function AdvancePaymentManagement({ isDark, onClose, cohostUserId }: AdvancePaymentManagementProps) {
  ...
  const fetchAdvances = async () => {
    ...
    if (cohostUserId) {
      ...
      if (cohostClients && cohostClients.length > 0) {
        const clientIds = cohostClients.map(c => c.id);
  ...
  const fetchClients = async () => {
    ...
    if (cohostUserId) {
      query = query.eq('parent_user_id', cohostUserId);
    }
  ...
  const addAdvancePayment = async () => {
    ...
    try {
      ...
      if (existingAdvance) {
        ...
        const { data: result, error: rpcError } = await supabase.rpc('apply_advance_simple_v2', {
          p_client_name: client.name,
        ...
        alert(`Advance payment added!\n\nDues cleared: ₹${res.dues_cleared}\nNet due now: ₹0\nAdvance stored: ₹${res.advance_stored}\n\nAdvance will be used for future meetings.`);
      }
