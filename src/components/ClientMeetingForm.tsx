...
export function ClientMeetingForm({ user, isDark, onMeetingAdded, selectedDate }: ClientMeetingFormProps) {
  ...
  const handleRecurringSubmit = async (recurringData: { endDate: string | null; noEndDate: boolean; exceptionDays: string[] }) => {
    if (!meetingName.trim() || !meetingId.trim() || !password.trim()) {
      alert('Please fill in all fields');
      return;
    }
    ...
    const isExcluded = recurringData.exceptionDays.includes(todayName);

    if (!isExcluded && newTemplate) {
      await supabase.from('meetings').insert({
        ...
        scheduled_date: selectedDate,
        is_recurring: true,
        recurring_template_id: newTemplate.id,
        auto_created: true,
        status: 'scheduled',
        attended: false
      });
    ...
    alert('✅ Meeting added to daily recurring list!' + (!isExcluded ? ' Today\'s meeting also created.' : ''));
    ...
    <>
      <form onSubmit={handleSubmit} className={`rounded-2xl shadow-xl p-6 border-2 ${
        ...
        <div className="space-y-4">
          ...
          <div className={`flex items-center gap-3 p-4 rounded-xl ${isDark ? 'bg-orange-900/20 border border-orange-500/30' : 'bg-orange-50 border border-orange-200'}`}>
            <input
              type="checkbox"
              id="instant"
              checked={isInstant}
            ...
            />
            <label htmlFor="instant" className={`text-sm font-semibold flex-1 ${isDark ? 'text-orange-300' : 'text-orange-700'}`}>
              Mark as Instant Meeting (appears at top with orange highlight)
            </label>
          </div>
