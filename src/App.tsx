...
function App() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const underscoreMatch = signupName.match(/^([A-Za-z]+)_(.+)$/);
    const hyphenMatch = signupName.match(/^([A-Za-z]+)-(.+)$/);

    if (underscoreMatch || hyphenMatch) {
      const match = underscoreMatch || hyphenMatch;
      const prefix = match![1].toUpperCase();
      const clientName = match![2];

      const { data: cohostData } = await supabase
        .from('users')
        .select('id, cohost_rate, is_cohost, cohost_prefix')
        .eq('is_cohost', true)
        .ilike('cohost_prefix', prefix)
        .maybeSingle();

      if (cohostData) {
        parentUserId = cohostData.id;
        cohostRate = cohostData.cohost_rate || 0;
        displayName = clientName;
      } else {
        alert(`Invalid prefix "${prefix}". No co-host found with this prefix.`);
        return;
      }
    }

    const finalPricePerMember = parentUserId ? cohostRate : signupPricePerMember;
    const emailForLogin = `${signupName.toLowerCase().replace(/\s+/g, '')}@client.junaid.com`;
      ...
      .insert([
        {
          ...
          role: 'client',
          price_per_member: finalPricePerMember,
      ...
      <div className="w-full max-w-md">
        <div className="bg-gradient-to-r from-gray-900 via-slate-800 to-gray-900 rounded-3xl shadow-2xl p-8 mb-6 border border-gray-700/50">
          ...
          <p className="text-gray-300 text-center text-sm font-light">
            {showSignup ? 'Create Your Account' : 'Professional Meeting Automation'}
          </p>
        </div>
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-slate-500 focus:ring-2 focus:ring-slate-200 outline-none transition-all duration-300"
                  placeholder="e.g., AJ_YourName or YourName"
                  required
                />
                <p className="text-xs text-gray-500 mt-1">
                  Use PREFIX_Name format if you have a referral code (e.g., AJ_Rahul)
                </p>
              </div>
