...
try {
  ...
  const { error: e1 } = await supabase.rpc('exec_sql', { sql_query: createExecSql });

  if (e1) {
    console.log('   Creating function directly...');
    const { data: d, error: e2 } = await supabase.from('pg_stat_statements').select('*').limit(1);
    console.log('   Function may already exist or using alternative method\n');
  } else {
    console.log('   ✅ Helper function ready\n');
  }

  console.log('Step 2: Dropping old constraint...');
  const { error: drop } = await supabase.rpc('exec_sql', {
    sql_query: 'ALTER TABLE meetings DROP CONSTRAINT IF EXISTS meetings_status_check'
  });
  console.log(drop ? '   ⚠️  ' + drop.message : '   ✅ Done\n');

  console.log('Step 3: Adding new constraint with all statuses...');
  const { error: add } = await supabase.rpc('exec_sql', {
